...
def make_pdf(filename: str, title: str, sections: list[tuple]) -> None:
    pdf = FPDF()
    pdf.add_page()
...
def make_contrat(fournisseur: str) -> None:
    make_pdf(
...
make_pdf(
    "procedures-achats.pdf",
    "Procedures Internes - Service Achats",
    [
        (
...
make_pdf(
    "contrat-fournisseur-type.pdf",
    "Contrat Cadre Fournisseur - Modele Type",
    [
        (
            "Article 1 - Objet",
            [
                f"Le present contrat definit les conditions generales d'achat entre {ACHETEUR.split(' - ', maxsplit=1)[0]} (Acheteur) et le Fournisseur.",
